...
class ScaleImageView @JvmOverloads constructor(
    ...
    /**
     * Callback called then ImageView was pulled up or down to screen top or bottom bound
     * */
    ...
    init {
        //background = ColorDrawable(ContextCompat.getColor(context, R.color.transparent_dark))
    }

    private fun updateBounds(values: FloatArray) {
        if (drawable != null) {
            bounds[values[MTRANS_X],
                    values[MTRANS_Y],
                    drawable.intrinsicWidth * values[MSCALE_X] + values[MTRANS_X]
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        if (doubleTapDetected) {
            ...
            // If image already scaled (start matrix don't same to current), return image to start matrix
            if (matrixValues[MSCALE_X] != startValues!![MSCALE_X]) {
                animateToStartMatrix()
            } else {
                // Otherwise zoom to doubleTapToZoomScaleFactor
    ...
    private fun disallowParentTouch() =
        currentScaleFactor > 1.0f || resetAnimator?.isRunning == true
